...
- **`hn` CLI** — one root binary, three mountable lanes:
  - `hn finance` — read-only market data: quotes · positions · net worth · fundamentals (SEC EDGAR) ·
    multiples · news · research · watch · screen. **No trading; observation only.**
...
```yaml
companies:
  - {name: Stripe, ats: greenhouse, token: stripe, tier: "Tier 1"}
filters:
