...
/// The default travel dashboard — a general trip command-center (every source already in the
/// toolkit, v0.40.0): the corpus trip-pipeline spine + home-base weather + the key-dates/sports
/// almanac + travel signals. Replaces the old single-trip view that went stale the day a trip ended.
    ...
    Dashboard {
        ...
        widgets: vec![
            w("pipeline", "Trip pipeline — the horizon", "table",
              hn_cmd(&["travel", "trips", "--json"]), "local60s", 4,
              Some("trips"), None, None),
            // No hardcoded city: `weather` defaults to the active weights' `conditions.home`
            // (pack-aware), and `title_path` surfaces the resolved place in the title (amber).
            Widget {
                title_path: Some("location".into()),
                ..w("weather", "Home-base weather", "table",
                    hn_cmd(&["travel", "weather", "--from", "{today}", "--to", "{today+4}", "--json"]),
