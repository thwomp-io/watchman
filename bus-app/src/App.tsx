...
function WatchmenBand({ wm, health }: { wm: WatchmenStatus | null; health: Health }) {
  const overall = !wm ? "ACQUIRING…"
    : wm.overall === "red" ? "ATTENTION" : "ALL SYSTEMS NOMINAL";
  return (
        ...
        {wm?.agents.map((a) => (
          <div className="vital" key={a.id}>
            ...
            <div className="vital-stat"><span className="stat-label">TODAY</span>
              <span className="stat-val">{a.market_day ? `${a.runs_today}/${a.expected_by_now}` : "—"}</span></div>
...
// (B) the default detail pane — a calm digest when no signal is selected.
function Digest({ wm, unread, agenda, health }: {
  wm: WatchmenStatus | null; unread: BusEvent[]; agenda: Agenda | null; health: Health;
}) {
  const pulse = wm?.agents.find((a) => a.id === "pulse");
  const alerts = unread.filter((e) => e.severity === "alert").length;
  const warns = unread.filter((e) => e.severity === "warn").length;
  const headline = unread.length === 0 ? "ALL QUIET"
    : alerts > 0 ? `${alerts} NEED${alerts === 1 ? "S" : ""} YOU`
    : `${unread.length} UNREAD`;
  const summary = unread.length === 0
    ? "no unread signals · the watchmen are standing by"
    : [alerts && `${alerts} alert`, warns && `${warns} watch`].filter(Boolean).join(" · ");
  const nextPrint = agenda?.prints?.[0];

  return (
    <div className={`digest glow-${health}`}>
      ...
      <dl className="agenda">
        ...
        {nextPrint && (
          <div><dt>next print</dt><dd>{nextPrint.symbol} · {nextPrint.days_out}d</dd></div>
        )}
        {agenda?.wash && (
          <div>
            <dt>Vest wash</dt>
            <dd>{agenda.wash.today_poisoned
              ? `poisoned · clear ${agenda.wash.next_clean_start ?? "—"}`
              : "clear"}</dd>
          </div>
        )}
        {pulse?.last_flags && (
          <div className="agenda-flags">
            <dt>last flag</dt><dd>{pulse.last_flags.split(";")[0]}</dd>
          </div>
        )}
      </dl>
      <p className="digest-foot">select a signal to inspect its payload</p>
    </div>
  );
}
...
function Inbox({ onUnread }: { onUnread: (n: number) => void }) {
  ...
  const reload = useCallback(async () => {
    ...
    onUnread(evs.filter((e) => !e.read_at).length);
  ...
  const health = harnessHealth(wm, unread);
  ...
  return (
    <div className="inbox">
      ...
      <div className="split">
        <ul className="feed bezel">
          {events.length === 0 && <li className="empty">NO SIGNALS — THE WATCHMEN ARE QUIET</li>}
          {events.map((e) => (
            <li key={e.id}
                className={[
              ...
              <span className="led" />
              <span className="lane-tag">{e.lane}</span>
        ...
        <aside className="detail bezel">
          {selected ? (
            ...
            <Digest wm={wm} unread={unread} agenda={agenda} health={health} />
          )}
        </aside>
      </div>
...
function sameRef(a: Ref, b: Ref): boolean {
  return a.zone === b.zone && a.doc === b.doc && a.viz === b.viz && a.dir === b.dir;
}
