                      ...
                      if (href?.startsWith("wiki:")) {
                        ...
                        return (
                          <a
                            ...
                            onClick={(e) => {
                              e.preventDefault();
                              // Route through the nav PRIMITIVE (not local select()) so the click pushes
                              // a history entry — `report` keeps `current` synced to the doc we're leaving,
                              // so navigate() stacks it and Back returns here (the bug was
                              // select() only firing report, which overwrites `current` in place → Back
                              // skipped the source doc). Same primitive doc_series uses to enter VAULT.
                              if (hit) nav.navigate({ zone: "vault", doc: hit.path });
                            }}
                          >
                            {children}
                          </a>
