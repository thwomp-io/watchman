---
tags: [bead, finance, equity]
---

# scale-u3 — Confirm the trading window stays open for the staged tranche sales

**Status:** in_progress · **Priority:** P2 · **Type:** task · **Assignee:** quant

> GENERATED from the beads db — a rendered view; edit via `bd`, never here.

## Details

| field | value |
| --- | --- |
| Created | 2026-07-06 · Casey Morgan |
| Updated | 2026-07-13 |
| Started | 2026-07-12 |
| Labels | finance, equity |
| Owner | casey@example.com |

## Description

The employer trading policy and the earnings calendar decide this, not the market: the window that opened 7/9 typically closes two weeks ahead of the quarterly print. Pin down the exact close date before any tranche-2 order rests — a GTC order that outlives the window is a policy violation, not a trade.

## Linked issues

**Parent** (1)
- [[scale-u1]] — CRWD unwind — staged diversification of the vested employer RSUs · open

**Blocks** (2)
- [[scale-u2]] — Tranche 1 — sell the 2024-08 deep-gain lot (60 sh, limit 395) · open
- [[scale-u4]] — Place the tranche-2 limit sale — the 2025-02 lot (55 sh) · open
