...
CONSOLE_LANES = (
    "backlog",
    "finance", "finance-prints", "finance-projections", "finance-holdings",
    "travel", "travel-landscape", "travel-conditions", "travel-calendar", "travel-calendar-big",
    "travel-visits",
    "market", "news", "plans", "tickets", "compare", "career",
)
# Lanes that USED to be compiled defaults and were retired — a stale pack file here would resurrect a
# dead tab under that persona (the concentration-unwind tab was retired;
# the `finance unwind` verb stays).
RETIRED_LANES = ("unwind",)
...
def test_pack_ships_the_full_console(pack: Path) -> None:
    ...
    for lane in RETIRED_LANES:
        assert not (pack / "dashboards" / f"{lane}.json").exists(), \
            f"{pack.name}: dashboards/{lane}.json is a retired tab — delete it (re-run emit_pack_dashboards)"
...
def test_pack_provides_every_dashboard_read_source(pack: Path) -> None:
    ...
    # the Ops ▸ Backlog tab + the Beads board viz read the beads export; ticket docs make the
    # board rows' "open ↗" links + the tile quick-look land somewhere (pre-generated — a bundled
    # install is read-only, so the sync can't create them at first render)
    assert (pack / ".beads" / "issues.jsonl").is_file(), \
        f"{pack.name}: no .beads/issues.jsonl — the Backlog tab + Beads board would be calm-empty"
    assert any((pack / "ops" / "beads").glob("*.md")), \
        f"{pack.name}: no ops/beads/*.md ticket docs — board links + tile quick-looks would 404"
    # the demo fixture must carry the anchor line so its dates stay eternally fresh
    first = (pack / ".beads" / "issues.jsonl").read_text().splitlines()[0]
    assert json.loads(first).get("anchor_date"), \
        f"{pack.name}: beads fixture missing the anchor_date meta line (would age out of the demo)"
    # the Finance ▸ Prints / Projections / Holdings tabs read the two agent-authored registries
    # (pack-overridable: `finance/print-scorecards.yaml` + `finance/projection-params.yaml`); absent,
    # the tabs render an honest empty — which a demo persona must never do
    assert (pack / "finance" / "print-scorecards.yaml").is_file(), \
        f"{pack.name}: no finance/print-scorecards.yaml — the Prints tab would be empty"
    assert (pack / "finance" / "projection-params.yaml").is_file(), \
        f"{pack.name}: no finance/projection-params.yaml — Projections + Holdings would appraise nothing"
    # the Finance ▸ Plans tab's event-plan panel (doc_series over finance/plans/)
    assert any((pack / "finance" / "plans").glob("*.md")), \
        f"{pack.name}: no finance/plans/*.md — the Plans-tab event-plan panel would be empty"
    # the Travel ▸ Landscape tab: the destination-characteristics matrix (File viz) + its how-to-read doc
    assert (pack / "travel" / "destinations" / "_meta" / "meta-ranking.json").is_file(), \
        f"{pack.name}: no travel/destinations/_meta/meta-ranking.json — the Landscape matrix would be empty"
    assert any((pack / "travel" / "destinations" / "_meta").glob("*.md")), \
        f"{pack.name}: no travel/destinations/_meta/*.md — the Landscape doc panel would be empty"
    # the Travel ▸ Visits tab (`trips --kind visit`) needs at least one hosting-visit doc
    assert any((pack / "travel" / "visits").glob("*.md")), \
        f"{pack.name}: no travel/visits/*.md — the Visits tab would be empty"
    # the Planning tab's candidate radar (`travel decision`) resolves the soonest live trip whose
    # FOLDER-NOTE carries visuals/compare-data.json — a flat trip doc can't carry one, so the widget
    # renders an honest-empty radar under that persona
    assert any((pack / "travel" / "trips").glob("*/visuals/compare-data.json")), \
        f"{pack.name}: no travel/trips/<slug>/visuals/compare-data.json — the Planning radar would be empty"
