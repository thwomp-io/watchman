...
class FinanceService:
    ...
    def multiples(self, symbol: str, *, cik: str | None = None, recent: int = 8) -> Multiples:
        """Valuation multiples (EV/EBITDA, P/E, P/S) + the full auditable component breakdown, from
        SEC EDGAR reported figures + a LIVE Alpaca price. GAAP-honest: unprofitable → "N/M", a missing
        XBRL tag → "unavailable" (named), never a fabricated number. `recent` ≥ 8 so the TTM assembly
        has at least 4 discrete quarters to draw from (income statements interleave quarter + YTD facts).

        Read-only observation surface for the screened-core engine + research profiles."""
    ...
    def watch(self, *, mark_seen: bool = True, news_limit: int = 4) -> WatchDigest:
        ...
        from harness.finance.providers.news_provider import fetch_edgar_filing_feed, fetch_rss
...
def _load_feeds() -> list[dict[str, str]]:
    ...
    return [
        {"name": str(f.get("name", "feed")), "url": str(f["url"])}
